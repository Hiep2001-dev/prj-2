            ...
            <div class="row">
                <div class="col-6 d-flex">
                    ...
                    @if ($actions['createExcel'])
                        <button class="ml-1 btn btn-success">Excel</button>
                    @endif
                </div>
            </div>
        </div>
                ...
                <div class="row">
                    <div class="mb-3 col-3">
                        <div class="mb-3 input-group">
                            ...
                            <input type="text" name="reservation" class="float-right form-control" id="reservation">
                        </div>
                    </div>
                    <div class="mb-3 col-sm-3">
                        <select class="form-select" aria-label="Default select example" name="status">
                            <option value="" {{ request()->status == '' ? 'selected' : '' }}>Trạng thái đơn hàng
                                (Tất cả)</option>
                            <option value="0" {{ request()->status == '0' ? 'selected' : '' }}>Chờ xử lý</option>
                            <option value="1" {{ request()->status == '1' ? 'selected' : '' }}>Đã xác nhận
                            </option>
                            <option value="1" {{ request()->status == '4' ? 'selected' : '' }}>Đang giao hàng
                            </option>
                            <option value="3" {{ request()->status == '3' ? 'selected' : '' }}>Đã nhận hàng
                            </option>
                    ...
                    <div class="mb-3 col-sm-3">
                        <select class="form-select" aria-label="Default select example" name="payment">
                            <option value="" {{ request()->payment == '' ? 'selected' : '' }}>Phương thức TT
                                (Tất cả)</option>
                            <option value="1" {{ request()->payment == '1' ? 'selected' : '' }}>Khi nhận hàng
                            </option>
                            <option value="2" {{ request()->payment == '2' ? 'selected' : '' }}>VNPAY
                            </option>
                    ...
                    <tr id="{{ $item->id }}" style="text-align: left;">
                        ...
                        @if ($actions['viewDetail'] || $actions['edit'] || $actions['delete'])
                            <td style="text-align: left;">
                                ...
                                @endif
                                @if ($actions['delete'])
                                    <form style="display: inline;" action="{{ route($routes['delete']) }}"
