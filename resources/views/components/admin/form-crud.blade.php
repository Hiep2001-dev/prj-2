            ...
            <x-admin-input-prepend label="{{ $field['label'] }}">
                        ...
                        @if (isset($field['list']))
                            @foreach ($field['list'] as $option)
                                <option value="{{ $option['value'] }}"
                                    @if (old($field['attribute'])) @php if (old($field['attribute']) == $option['value']) echo "selected"; @endphp
                @elseif(isset($field['value']))
                  @php if ($field['value'] == $option['value']) echo "selected"; @endphp @endif>
                                    {{ $option['text'] }}</option>
                            @endforeach
                        @endif
                    </select>
                @elseif($field['type'] == 'file')
                    <div class="custom-file">
                        <input type="file" name="{{ $field['attribute'] }}" class="form-control inputFile__js"
                            id="{{ $field['attribute'] }}">
                    </div>
                @else
                    <input id="{{ $field['attribute'] }}" type="{{ $field['type'] }}"
                        value="{{ old($field['attribute']) ?? ($field['value'] ?? null) }}"
                        name="{{ $field['attribute'] }}" autocomplete="{{ $field['autocomplete'] ?? null }}"
                        {{ $field['other'] ?? null }}
                        @if (isset($field['format_phone'])) data-inputmask='"mask": "999 999-9999"' data-mask @endif
                        class="form-control">
                @endif
