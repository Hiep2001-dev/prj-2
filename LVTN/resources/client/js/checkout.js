...
function getFee() {
    ...
    $.ajax({
        ...
        $.ajax({
            ...
            $('#fee').text(new Intl.NumberFormat().format(fee) + ' VND');
            $('#total-order').text(new Intl.NumberFormat().format(fee + totalProduct) + ' VND');
...
document.getElementById('apply-voucher').addEventListener('click', function (event) {
    ...
    if (!code) {
        document.getElementById('discount-message').innerText =
            "Bạn không nhập mã giảm giá, tổng tiền giữ nguyên.";
    ...
    // Lấy giá trị tổng tiền sản phẩm từ phần tử #total-product
    const totalProductText = document.getElementById('total-product').innerText.trim();
    const totalProduct = parseFloat(totalProductText.replace(/,/g, "").replace(" VND", ""));

    // Lấy giá trị phí vận chuyển từ phần tử #fee
    const shippingFeeText = document.getElementById('fee').innerText.trim();
    const shippingFee = parseFloat(shippingFeeText.replace(/,/g, ""));
    ...
    fetch('/apply-voucher', {
        ...
        .then(data => {
            if (data.error) {
                ...
                document.getElementById('discount-message').innerText = data.error;
                document.getElementById('discount-message').style.color = "red";
                document.getElementById('discount-message').style.display = "block";
            } else {
                // Cập nhật tổng tiền và hiển thị thông báo thành công
                document.getElementById('discount-message').innerText = "Thêm mã giảm giá thành công!";
                document.getElementById('discount-message').style.color = "green";
                document.getElementById('discount-message').style.display = "block";

                // Cập nhật tổng đơn hàng mới (bao gồm phí vận chuyển và giảm giá)
                document.getElementById('total-order').innerText = new Intl.NumberFormat().format(data
                    .newTotal) + ' VND';
            }
        })
        .catch(error => {
